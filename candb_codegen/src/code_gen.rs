...
impl CodeGenerator {
            ...
            for frame in frames {
                for signal in &frame.signals {
                    if let DataType::Enum(enum_) = &signal.dt {
                        if let Some(existing) = enum_tab.iter_mut().find(|x| x.name == signal.name)
                        {
    ...
    pub fn code_gen(&self, layer_name: &str) -> std::fmt::Result {
        self.enum_gen();
    ...
    pub fn generate_ecu(&self, name: &String, frames: &[Frame]) {
        ...
        let arb_int_req = frames.iter().any(|x| {
            x.signals.iter().any(|sig| {
                (sig.len != 1 && sig.len != 8 && sig.len != 16 && sig.len != 32 && sig.len != 64)
        ...
        let enum_import_req = frames.iter().any(|x| {
            x.signals
                .iter()
                .any(|sig| matches!(sig.dt, DataType::Enum(_)))
        });
        ...
        for frame in frames {
            ...
            for signal in &frame.signals {
                let (derrive_args, dt) = match &signal.dt {
                    DataType::Bool => (
                        format!("#[bit({}, rw)]", 63 - signal.offset),
                        "bool".to_string(),
                    ),
                    DataType::Int {
                        offset: _,
            ...
            let _ = writeln!(
                s,
                "\tconst CAN_ID: StandardId = unsafe {{ StandardId::new_unchecked(0x{:04X}) }};",
                frame.id
            );
            let _ = writeln!(
                s,
            ...
            let n_bytes = frame
                .signals
                .last()
                .map(|x| ((x.len + x.offset) / 8) as usize)
                .unwrap_or(8);
            let _ = writeln!(
                s,
                "\t\tlet bytes = &self.raw_value().to_be_bytes()[..{n_bytes}];"
            );
            let _ = writeln!(s, "\t\tMessage::new(MessageBuilder {{");
            let _ = writeln!(s, "\t\t\tid: Id::Standard(Self::CAN_ID),");
            let _ = writeln!(
                s,
                "\t\t\tframe_type: FrameType::Classic(ClassicFrameType::Data(bytes)),"
            );
